...
class Config(object):
    ...
    def max_valid_samples_task2(self, max_samples):
        assert (
            self._max_valid_sample_task2s is not None), 'max_valid_samples_task2 does not exist.'
    ...
    def get_model(self, ngpus, pretrained=False, epoch=-1, dataParallel=False):
        ...
        if pretrained:
            restore_path = tools.get_checkpoint(
...
class Trainer(object):
    ...
    def __call__(self, x, y, update_weights=True,
                 activations=False, gradients=False):
        self.model.train()
...
def train_split(config_file, pretrained=False, restore_epoch=-1,
    ...
    config = Config(config_file=config_file)
    model, ckpt_data = config.get_model(
        ngpus=ngpus, pretrained=pretrained, epoch=restore_epoch)
    ...
    valid_num_steps = max(len(validator1.data_loader),
                          len(validator1.data_loader))
    valid_step_freq = int(train_num_steps * valid_freq)
    stop_epoch = start_epoch + epochs
    for epoch in trange(start_epoch, stop_epoch + 1,
                        initial=start_epoch, desc='epoch'):
        # for train_step, train_batch in enumerate(tqdm(trainer.data_loader,
        # desc='train')):
        ...
        for train_step, (train_batch1, train_batch2) in enumerate(tqdm(itertools.zip_longest(trainer1.data_loader,
            ...
            if train_batch1 is not None:
                ...
                train_loss1, _ = trainer1(
                    x=x_train1, y=y_train1)  # y_pred_train1
            ...
            if train_batch2 is not None:
                ...
                train_loss2, _ = trainer2(
                    x=x_train2, y=y_train2)  # y_pred_train2
            ...
            writer.add_scalar(
                'meta/learning_rate',
                trainer1.scheduler.get_lr()[0],
                global_step)
            writer.add_scalar(
                'meta/progress',
                (train_step + 1.0) / train_num_steps,
                global_step)
            writer.add_scalar(
                'meta/train-sec-per-iter',
                train_sec_per_iter,
...
def gradients_and_activations(activations=False, gradients=False,
                              max_batches=None, validator=None, trainer=None, notebook=False):
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='act/grad')):
        ...
        if trainer is not None:
            ...
            assert (len(all_activations[step]) == len(
                all_gradients[step])), "Something is wrong with activations or gradients, tell Julio lol"
...
def get_subgroup_performances(subgroups_file, y_true, y_pred, losses):
    ...
    for subgroup in np.sort(subgroup_ids):
        ...
        accuracy = metrics.accuracy_score(
            subgroups[subgroup]['y_true'],
            subgroups[subgroup]['y_pred'])
        subgroup_performances.append((loss, accuracy))

    return subgroup_performances


def consolidateTask(trainer, task, ngpus=None,
                    consolidateModel=False, dataParallel=True):
    ...
    if consolidateModel:
        ...
        trainer.model.classifier[-1] = torch.nn.Linear(
            in_features, num_new_classes)
        if dataParallel:
