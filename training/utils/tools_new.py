...
def save_checkpoint(ckpt_data, ckpt_dir, max_count=-1, keep=None):
    ...
    keep = set() if keep is None else set(
        f'epoch_{epoch:02d}.pth.tar' for epoch in keep)
    ...
    if max_count == -1:
        ...
        if keep:
            checkpoints = [ckpt for ckpt in checkpoints if ckpt not in keep]
        removable = len(checkpoints)
        if removable + 1 <= max_count:
            torch.save(
                ckpt_data,
                os.path.join(
                    ckpt_dir,
...
def get_model(name, num_classes, ngpus, split_index=None,
              dataParallel=False, scale=None, fc_affected=None):
    modelClass = getattr(models, name)

    if split_index is None:
        if scale is None:
            modelObject = modelClass(num_classes=num_classes)
        elif fc_affected is None:
            modelObject = modelClass(num_classes=num_classes, scale=scale)
        else:
            modelObject = modelClass(
                num_classes=num_classes,
                scale=scale,
                fc_scaling=fc_affected)
    else:
        if scale is None and fc_affected is None:
            modelObject = modelClass(
                num_classes=num_classes,
