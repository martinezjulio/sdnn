...
def get_pred_task_dual_net(network_name, lesion_name,
                           version_name, task_sort_name, task_nonsort_name, layer):

    # 'INDEX_12/SORTEDBY_data_car/PERCENT_0.20/y_pred'

    filename = './lesions/' + network_name + '/' + lesion_name + '/drop_percents_records/VERSION' + \
        version_name + '/EVALUATION_TASK_' + task_sort_name + \
        '/predictions/PARAM_GROUP/predictions.jsonl'
    ...
    key = 'INDEX_' + layer + '/SORTEDBY_' + \
        task_sort_name + '/PERCENT_' + perc + '/y_true'
    if key in pred_data:
        y_true = pred_data[key]

    key = 'INDEX_' + layer + '/SORTEDBY_' + \
        task_sort_name + '/PERCENT_' + perc + '/y_pred'
    if key in pred_data:
    ...
    key = 'INDEX_' + layer + '/SORTEDBY_' + \
        task_nonsort_name + '/PERCENT_' + perc + '/y_true'
    if key in pred_data:
        y_true = pred_data[key]

    key = 'INDEX_' + layer + '/SORTEDBY_' + \
        task_nonsort_name + '/PERCENT_' + perc + '/y_pred'
    if key in pred_data:
        y_pred = pred_data[key]
    ...
    filename = './lesions/' + network_name + '/' + lesion_name + '/drop_percents_records/VERSION' + \
        version_name + '/EVALUATION_TASK_' + task_nonsort_name + \
        '/predictions/PARAM_GROUP/predictions.jsonl'
...
def get_pred_dual_net(network_name, lesion_name,
                      task_sort_name, task_nonsort_name, layer):

    version = 'evalPred'

    # 'INDEX_12/SORTEDBY_data_car/PERCENT_0.20/y_pred'

    filename = './lesions/' + network_name + '/' + lesion_name + '/drop_percents_records/VERSION__' + \
        version + '/EVALUATION_TASK_' + task_sort_name + \
        '/predictions/PARAM_GROUP/predictions.jsonl'
    ...
    filename = './lesions/' + network_name + '/' + lesion_name + '/drop_percents_records/VERSION__' + \
        version + '/EVALUATION_TASK_' + task_nonsort_name + \
        '/predictions/PARAM_GROUP/predictions.jsonl'
...
def get_pred_dual_net_old(network_name, lesion_name,
                          task_sort_name, task_nonsort_name, layer):
...
def get_bootstrap_prop_acc(task_true, task_pred):
    ...
    for iBoot in range(n_boot):
        ...
        # bootstrap classes
        bootstrap_classes = np.random.choice(
            rand_classes, size=n_classes, replace=True)

        for i, iClass in enumerate(bootstrap_classes):
            class_images = np.where(task_true[0, :] == iClass)[0]

            bootstrap_images = np.random.choice(
                class_images, size=class_images.shape[0], replace=True)
            ...
            if i == 1:
                ...
                y_true_boot = np.hstack((y_true_boot, np.ones(
                    shape=(class_images.shape[0],)) * iClass))
                y_pred_base_boot = np.hstack(
                    (y_pred_base_boot, task_pred[0, bootstrap_images]))
                y_pred_boot_1 = np.hstack(
                    (y_pred_boot_1, task_pred[1, bootstrap_images]))
                y_pred_boot_2 = np.hstack(
                    (y_pred_boot_2, task_pred[2, bootstrap_images]))

        acc_1 = np.where(y_true_boot == y_pred_boot_1)[
            0].shape[0] / y_true_boot.shape[0]
        acc_2 = np.where(y_true_boot == y_pred_boot_2)[
            0].shape[0] / y_true_boot.shape[0]
        acc_base = np.where(y_true_boot == y_pred_base_boot)[
            0].shape[0] / y_true_boot.shape[0]
        # print(acc)
    ...
    for iBoot in range(n_boot):
        ...
        for i, iClass in enumerate(bootstrap_classes):
            ...
            if i == 1:
                ...
                y_true_boot = np.hstack((y_true_boot, np.ones(
                    shape=(class_images.shape[0],)) * iClass))
                y_pred_boot = np.hstack(
                    (y_pred_boot, y_pred[bootstrap_images]))

        acc = np.where(y_true_boot == y_pred_boot)[
            0].shape[0] / y_true_boot.shape[0]
