...
def init_trace_tensor(dim):
    #return torch.randn(1,dim,1,1)
    return torch.ones(1,dim,1,1)
    #return torch.FloatTensor(1, dim, 1, 1).uniform_(0, 1).round()

class VGG16SubNetwork(torch.nn.Module):
    def __init__(self, model=None):
        super(VGG16SubNetwork, self).__init__()
        ...
        self.t0 = torch.nn.Parameter(init_trace_tensor(W0.shape[1]), requires_grad=True)
        ...
        self.t2 = torch.nn.Parameter(init_trace_tensor(W2.shape[1]), requires_grad=True)
        ...
        self.t5 = torch.nn.Parameter(init_trace_tensor(W5.shape[1]), requires_grad=True)
        ...
        self.t7 = torch.nn.Parameter(init_trace_tensor(W7.shape[1]), requires_grad=True)
        ...
        self.t10 = torch.nn.Parameter(init_trace_tensor(W10.shape[1]), requires_grad=True)
        ...
        self.t12 = torch.nn.Parameter(init_trace_tensor(W12.shape[1]), requires_grad=True)
        ...
        self.t14 = torch.nn.Parameter(init_trace_tensor(W14.shape[1]), requires_grad=True)
        ...
        self.t17 = torch.nn.Parameter(init_trace_tensor(W17.shape[1]), requires_grad=True)
        ...
        self.t19 = torch.nn.Parameter(init_trace_tensor(W19.shape[1]), requires_grad=True)
        ...
        self.t21 = torch.nn.Parameter(init_trace_tensor(W21.shape[1]), requires_grad=True)
        ...
        self.t24 = torch.nn.Parameter(init_trace_tensor(W24.shape[1]), requires_grad=True)
        ...
        self.t26 = torch.nn.Parameter(init_trace_tensor(W26.shape[1]), requires_grad=True)
        ...
        self.t28 = torch.nn.Parameter(init_trace_tensor(W28.shape[1]), requires_grad=True)
        ...
        self.t31 = torch.nn.Parameter(init_trace_tensor(W31.shape[1]), requires_grad=True)
        ...
        self.t34 = torch.nn.Parameter(init_trace_tensor(W34.shape[1]), requires_grad=True)
        ...
        self.t37 = torch.nn.Parameter(init_trace_tensor(W37.shape[1]), requires_grad=True)  
    ...
    def forward(self, x, activations=False, gradients=False):
        ...
        for i in range(38):
            if (i in conv_layers):
                ...
                trace  = getattr(self, 't' + str(i))
                
                trace_activation = torch.nn.functional.relu6(input=trace*6.0)/6.0
                trace_weight     = torch.mul(input=trace_activation,other=weight) 
